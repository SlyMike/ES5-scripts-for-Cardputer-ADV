...
// ES5 Digital Clock with Manual Time Set + Colour Options for Cardputer ADV (Bruce v1.12)
...
function drawClock(timeStr){
  ...
  if (editMode){
    ...
    var unitLen = 2; // HH or MM or SS
    var ux = x + (unitStart * charWidth);
    var uw = unitLen * charWidth;
    display.drawFillRect(ux, underlineY, uw, 3, textColor);
  }
...
function handleKeys(){
  ...
  for (i=0;i<ks.length;i++){
    ...
    if (k==="B"||k==="b"){ bgIndex=(bgIndex+1)%bgColors.length; bgColor=bgColors[bgIndex]; redrawAll(); continue; }
