...
// ES5 Bruce JS – GPIO Pulse Tester v4 (Final)
...
function startPulse(){
  pulseActive = true;
  pulseStart = now();
  gpio.digitalWrite(SHUTTERPIN,true);
  statusText = "Pulsing";
  clearScreen(BLUE);
  drawHud();
}
...
function drawHud(){
  setTextEnv();
  printAt(10,10,"GPIO Pulse Tester");
  printAt(10,30,"Pin: G"+SHUTTERPIN+" | Status: "+statusText);
  printAt(10,50,batteryText());
}
...
function handleKeys(){
  ...
  for(var i=0;i<ks.length;i++){
    ...
    if(k==="C"||k==="c"){ controlsVisible=!controlsVisible; clearScreen(BG); if(controlsVisible){ drawControls(); } else { clearScreen(BG); drawHud(); }}
    if(controlsVisible){ continue; }
    if(k==="P"||k==="p"){ if(!pulseActive) startPulse(); }
    if(k==="G"||k==="g"){ if(!pulseActive){ pinIndex=(pinIndex+1)%gpioPins.length; setPinSafe(gpioPins[pinIndex]); clearScreen(BG); drawHud(); }}
  }
}
