...
// ---- State ----
var balls = [];                 // [{x,y,vx,vy,r,ci,px,py}]
var MAX_BALLS = 9;
var running = true;
var showHud = true;
...
function hudFpsOnly() {
  if (!showHud) return;
  // small area just for FPS number
  display.drawFillRect(32, 0, 40, HUD_H, BG);
  if (typeof display.setCursor === "function") display.setCursor(34, 2);
  if (typeof display.setTextColor === "function") display.setTextColor(WHITE);
  if (typeof display.setTextSize === "function") display.setTextSize(1);
  display.print(fps);
}
...
function showControls(on) {
  ...
  if (controlsVisible) {
    ...
    display.fill(BG);
    // redraw all balls at their current positions
...
function erasePrevBall(b){ display.drawFillCircle(b.px, b.py, b.r, BG); }
...
function makeBall(){
  ...
  return {x:x,y:y,vx:vx,vy:vy,r:r,ci:ci,px:x,py:y};
}
...
function physicsStep(b){
  // advance to new position; clamp & bounce
  var nx = b.x + b.vx, ny = b.y + b.vy;

  if (nx - b.r < 0){ nx = b.r; b.vx = -b.vx; }
  else if (nx + b.r >= W){ nx = W - 1 - b.r; b.vx = -b.vx; }

  if (ny - b.r < 0){ ny = b.r; b.vy = -b.vy; }
  else if (ny + b.r >= H){ ny = H - 1 - b.r; b.vy = -b.vy; }

  // update previous pos for next frame erase
  b.px = b.x; b.py = b.y;
  b.x = nx;   b.y = ny;

  // optional: color tick when bouncing (cost is small)
  // (skip here to minimize draw churn)
}
...
function applySizeMode(newMode){
  ...
  for (i=0;i<balls.length;i++){
    ...
    b.r = r;
    b.px = b.x; b.py = b.y; // ensure erase uses current
    drawBall(b);
  }
  hudInitial();
}
...
function handleKeys(){
  ...
  for (i=0;i<ks.length;i++){
    ...
    // HUD toggle
    if (k==="H" || k==="h"){ showHud=!showHud; if (showHud) hudInitial(); else display.drawFillRect(HUD_X, HUD_Y, HUD_W, HUD_H, BG); continue; }

    // Controls toggle
    if (k==="C" || k==="c"){ showControls(!controlsVisible); continue; }

    // Pause keys while controls visible
...
// ---- Main loop (two-phase render) ----
var frameMs = 33; // cap ~30 FPS to reduce SPI artifacts
var lastHudFpsTs = now();

while (running){
  if (!controlsVisible) {
    // Phase A: erase everything at previous positions
    var i;
    for (i=0;i<balls.length;i++){ erasePrevBall(balls[i]); }
    // Phase B: physics update (positions & bounce)
    for (i=0;i<balls.length;i++){ physicsStep(balls[i]); }
    // Phase C: draw everything at new positions
    for (i=0;i<balls.length;i++){ drawBall(balls[i]); }
  }

  handleKeys();

  // FPS (text update throttled to once per second)
  frames++;
  var t = now();
  if (t - lastFpsTs >= 1000){
    fps = frames; frames = 0; lastFpsTs = t;
    hudFpsOnly(); // small band update only for FPS
  }
